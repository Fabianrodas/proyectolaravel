            ...
            <a href="{{ route('messages.index', ['conversation_id' => $conversation->id]) }}"
              ...
              <div class="d-flex align-items-center p-3 border-bottom hover-bg-light">
                ...
                <div>
                  ...
                  <small class="text-muted">{{ $lastMessage ? $lastMessage->content : 'No messages yet' }}</small>
                </div>
              </div>
            </a>
          @empty
            ...
            @if($otherUser)
              ...
              <div class="message-box flex-grow-1 overflow-auto">
                @foreach ($selectedConversation->messages as $message)
                  <div class="d-flex message {{ $message->sender_id === auth()->id() ? 'sent' : 'received' }}">
                    @if($message->sender_id !== auth()->id())
                      <img src="{{ asset($message->sender->image ?? '/storage/images/default.jpg') }}"
              ...
              <form action="{{ route('messages.store') }}" method="POST" class="d-flex align-items-center mt-3 gap-2">
                @csrf
